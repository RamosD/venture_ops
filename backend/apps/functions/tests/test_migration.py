"""Teste da migração inicial do módulo functions (aplica, reverte, sem drift)."""
...
TABLE = "functions_functionprofile"


class FunctionsMigrationTests(TransactionTestCase):
    ...
    def test_migration_applies_and_reverts(self):
        ...
        self._migrate([("functions", None)])
        self.assertFalse(self._table_exists())
        self._migrate([("functions", "0001_initial")])
        self.assertTrue(self._table_exists())
